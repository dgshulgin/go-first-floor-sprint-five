...
// Формула расчета:
// преодоленная_дистанция_за_тренинировку_км / время_тренировки_часах
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	meanSpeed := t.distance()
	hours := t.Duration.Hours()
	return meanSpeed / hours
}
...
type CaloriesCalculator interface {
	...
	//возвращает кол-во КИЛОкалорий израсходованных
	Calories() float64
...
func (r Running) Calories() float64 {
	...
	subFormula := CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift

	calories := subFormula * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return calories
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	subFormulaWeight := CaloriesWeightMultiplier * w.Weight

	meanSpeed := w.meanSpeed() * KmHInMsec // m/s
	subFormulaSpeed := math.Pow(meanSpeed, 2.0) / (w.Height / CmInM)

	subFormulaSpeedWeighted := subFormulaSpeed * CaloriesSpeedHeightMultiplier * w.Weight

	calories := (subFormulaWeight + subFormulaSpeedWeighted) * w.Duration.Hours() * MinInHours
	return calories
}
...
func (s Swimming) Calories() float64 {
	...
	subFormulaSpeed := meanSpeed + SwimmingCaloriesMeanSpeedShift
	calories := subFormulaSpeed * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return calories
}
